package io.cockroachdb.workload.common.util;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

public class Multiplier {
    private static final Pattern DECIMAL_PATTERN = Pattern.compile("^[+-]?([0-9]+\\.?[0-9]*|\\.[0-9]+)\\s?([kKmMgG]+)");

    private static final Pattern INTEGER_PATTERN = Pattern.compile("^[+-]?([0-9]+)\\s?([kKmMgG]+)");
        ...
        while (matcher.find()) {
            ...
            switch (token) {
                ...
                case "G":
                    ...
                    break;
                default:
                    throw new NumberFormatException("Invalid multiplier " + token);
            }
        }
        if (value == 0) {
